...
import { DatabaseList, databaseNames } from "./database-list";
...
async function portAll(
  ...
  await Promise.all(
    Object.entries(inputList).map(([name, file]) =>
      portFile(file, outputList[name])
    )
  );
}
...
export async function parseGTFS(
  ...
  if (inputIsPath && (await fs.stat(inputFile as string)).isDirectory()) {
    ...
    if (inputIsPath) {
      ...
      buffer = await fs.readFile(inputFile as string, null);
