import {
	...
	Calendar, FareAttribute, Shape, Frequency,
	Transfer, FeedInfo, CalendarDate, FareRule,
} from 'query-pouch-gtfs';
import {
	trip, stopTime, frequency, transfer,
	calendarDate, shapePoint, stop,
} from 'query-pouch-gtfs'
...
export const transformers: { [name: string]: Transformer } = {
	...
	stops(doc): Stop {
		...
		if (doc.wheelchair_boarding) stopDoc.wheelchair_boarding = toInt(doc.wheelchair_boarding);
		return stopDoc;
	},
	routes(doc): Route {
